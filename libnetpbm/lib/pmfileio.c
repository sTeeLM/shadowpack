...
-----------------------------------------------------------------------------*/
    ...
    for (attempts = 0, gotFile = FALSE, error = FALSE;
        ...
        if (rc == NULL)
            ...
            rc = _open(filenameBuffer, tempFileOpenFlags(),
...
makeTmpfileWithTemplate(const char *  const filenameTemplate,
    ...
    filenameBuffer = _strdup(filenameTemplate);
...
-----------------------------------------------------------------------------*/
    while (unlinkListP) {
        ...
        _close(firstEntryP->fd);
        _unlink(firstEntryP->fileName);
...
pm_tmpfile(void) {
    ...
    arrangeUnlink(tmpfileNm, _fileno(fileP));
...
-----------------------------------------------------------------------------*/
    ...
    */
    _S_IFCHR;
    stat_rc = fstat(_fileno(original_file), &statbuf);
    if (stat_rc == 0 && (S_IFREG & statbuf.st_mode))
        seekable = TRUE;
    else 
        seekable = FALSE;
