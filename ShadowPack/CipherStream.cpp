...
CCipherStream::CCipherStream(CStream * pLower):
m_pLower(pLower)
{
...
BOOL CCipherStream::Read(LPVOID pBuffer, ULONGLONG nSize, CProgress & Progress, CPackErrors & Error)
{
	ULONGLONG nOffset = m_pLower->GetOffset();
...
BOOL CCipherStream::Write(const LPVOID pBuffer, ULONGLONG nSize, CProgress & Progress, CPackErrors & Error)
{
	BOOL bRet = FALSE;
	BYTE Buffer[4096], *p = (LPBYTE)pBuffer;
	ULONGLONG nLeft = nSize, nBatch;
	ULONGLONG nOffset = 0L;
...
ULONGLONG CCipherStream::GetOffset()
{
