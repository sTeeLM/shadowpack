...
TIFFFillStripPartial( TIFF *tif, int strip, tmsize_t read_ahead, int restart )
{
	...
	register TIFFDirectory *td = &tif->tif_dir;
        ...
        */
        to_read = tif->tif_rawdatasize - (size_t) unused_data;
        if( (uint64) to_read > td->td_stripbytecount[strip] 
            - tif->tif_rawdataoff - tif->tif_rawdataloaded )
        {
                to_read = (size_t)(td->td_stripbytecount[strip]
                        - tif->tif_rawdataoff - tif->tif_rawdataloaded);
        }
	...
	assert((tif->tif_flags&TIFF_BUFFERMMAP)==0);
        ...
        tif->tif_rawdataoff = tif->tif_rawdataoff + tif->tif_rawdataloaded - (size_t) unused_data ;
        tif->tif_rawdataloaded = (size_t) unused_data + to_read;
