        ...
        int64          inputBytes ) 
{
    ...
    if ( (context -> count [0] += uint32(inputBytes << 3)) < (inputBytes << 3) )
    context -> count [1]++;
    context -> count [1] += uint32(inputBytes >> (32 - 3));
    ...
    memcpy ( (context -> buffer) + byteIndex, input + i, size_t(inputBytes - i) );
}
